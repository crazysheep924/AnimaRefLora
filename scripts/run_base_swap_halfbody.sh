#!/usr/bin/env bash
# Base-swap blog section: one new half-body scene (neon night wink + finger
# heart) generated twice per character — once on the official Anima base,
# once on the WAI Anima community finetune — with the same 500K AnimaRefLoRA
# checkpoint, seed, and prompt. Outputs go straight to blog assets.
set -euo pipefail
...
RESULTS="${RESULTS:-${REPO}/blog/assets/results/base_swap}"
...
scene_prompt="upper body, bust shot, from slightly below, looking at viewer, playful wink, one eye closed, cheerful grin, finger heart, neon night street background, colorful neon signs bokeh, rim lighting, vibrant colors, detailed face and hair"

bases=(anima_base wai_base)
base_files=(anima-base-v1.0.safetensors waiANIMA_v10Base10.safetensors)

infer() {
  local dit_file="$1" ref_mount="$2" output="$3" prompt="$4" seed="$5"
...
for base_i in "${!bases[@]}"; do
  base="${bases[$base_i]}"
  dit="${base_files[$base_i]}"
  for char_i in "${!characters[@]}"; do
    character="${characters[$char_i]}"
    seed=$((5000 + char_i * 100))
    prompt="1girl, solo, ${scene_prompt}, ${appearance[$char_i]}, ${original_outfit[$char_i]}"
    infer "${dit}" "${REFS}/single_${character}" "${RESULTS}/${base}/${character}" "${prompt}" "${seed}"
  done
done

echo "=== base-swap half-body DONE -> ${RESULTS} ==="
